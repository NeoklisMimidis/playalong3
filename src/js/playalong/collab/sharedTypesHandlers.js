...
import { _colorizeTableSelections, editChord } from "../../annotation-tools/right-toolbar-tools";
import { mainWaveform, wavesurfer } from "../../audio-player";
import { MARKER_LABEL_SPAN_COLOR } from "../../config";
import { annotationList, chordEditor, disableAnnotationListAndDeleteAnnotation, toolbarStates } from "../../annotation-tools";
import { COLLAB_CHORD_SELECTION_TIPPY_PROPS, tooltips } from "../../components/tooltips";
import { addMarkerAtTime, updateMarkerDisplayWithColorizedRegions } from "../../audio-player/render-annotations";
...
export function handleSharedBTMarkersEvent (collabEditedMarker, key) {
  ...
  switch(collabEditedMarker.status) {
    //final values
    case 'edited': handleMarkerChordEditing(collabEditedMarker); 
      break;
    case 'added': handleMarkerAddition(collabEditedMarker);
      break;
    case 'deleted': handleMarkerDeletion(collabEditedMarker);
    case 'moved': handleMarkerMoving(collabEditedMarker, key);
      break;
    case 'added, edited': {
    ...
    case 'added, moved, edited': handleMarkerMoving(collabEditedMarker, key); //after handleMarkerMoving call --> added, edited
      break;
  }
}
...
function handleMarkerChordEditing (collabEditedMarker) {
  ...
  const correspondingMarker = wavesurfer.markers.markers
    .find(m => m.time == collabEditedMarker.time);
...
function handleMarkerDeletion (collabEditedMarker) {
  const markerToBeDeleted = wavesurfer.markers.markers
    .find(m => m.time == collabEditedMarker.time);
...
function handleMarkerAddition (collabEditedMarker) {
  ...
  //i.e. when an added (and edited) marker is moved and the shared marker object is updated in handleMarkerMoving
  const existingMarker = wavesurfer.markers.markers
    .find(m => m.time === collabEditedMarker.time);
  if (existingMarker)
    return;
...
function handleMarkerMoving (collabEditedMarker, key) {
  ...
  if ( status.includes('to be moved') ) { //editor drags

    //removing the marker that editor moves
    const markerToBeMarked = allMarkers
      .find(m => m.time === collabEditedMarker.time)
  ...
  } else if ( status.includes('moved') ) { //editor drops

    const markerToBeRemoved = allMarkers.find(m => m.time == metadata.timeOfMarkerToBeRemoved);
    ...
    //removing old marker
    markerToBeRemoved
      ? wavesurfer.markers.remove(markerToBeRemoved)
      : null; 
    ...
    if ( status.includes('added') ) {
      ...
      window.sharedBTMarkers.set(`${key}`, {time: collabEditedMarker.time, status: newStatus, metadata}); 
    }
  }
}
