...
async function setupWorker(sab, sampleRate) {
  await URLFromFiles([
    'audioworklet-to-worker/js/wav-writer.js',
    'audioworklet-to-worker/lib/ringbuffer/index.js',
  ]).then(e => {
    ...
    worker.onmessage = async function (e) {
      switch (e.data.command) {
        ...
        case 'audioBufferCurrentUpdated': {
          ...
          // draw waveform
          const canvas = document.querySelector('#waveformMB');
          let drawer = new WaveformDrawer(
            audioBufferNode.buffer,
            canvas,
            'green'
          );
...
URLFromFiles([
  'audioworklet-to-worker/js/recorder-worklet.js',
  'audioworklet-to-worker/lib/ringbuffer/index.js',
]).then(e => {
